...
test(description: 'EncryptService est instantiable', closure: function (): void {
    expect($this->service)->toBeInstanceOf(class: EncryptService::class);
});

test(description: 'Encrypt renvoie une chaîne non vide', closure: function (): void {
    $result = $this->service->encrypt(data: 'test data', privateKey: 'secret key');
    expect($result)->toBeString()->not->toBeEmpty();
});

test(description: 'Le chiffrement avec la même entrée produit une sortie différente en raison d\'une IV aléatoire.', closure: function (): void {
...
test(description: 'Les données à chiffrer ne peuvent pas être vides.', closure: function (): void {
...
test(description: 'La clé privée ne peut pas être vide.', closure: function (): void {
...
test(description: 'Les données cryptées contiennent IV', closure: function (): void {
    $result = $this->service->encrypt(data: 'test data', privateKey: 'secret key');
    $decoded = base64_decode(string: $result);
    expect($decoded)->toContain('::');
});

test(description: 'La methode CIPHER est disponible', closure: function (): void {
