...
/*test(description: 'Vérifie que les données reçues sont bien celles attendues et que l\'utilisateur possède le role ROLE_ADMIN', closure: function () use ($entityName, $uri,) {
    $testCase = new ApiTestCase(name: $entityName);
    try {
        $data = $testCase->getApiResponse(uri: $uri);
